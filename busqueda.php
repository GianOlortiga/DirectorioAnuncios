...
<!DOCTYPE html>
<html lang="es">
<?php include("estructura/head-busqueda.php"); ?>
<body>
	<?php include("estructura/header.php");?>
		<div class="container" id="nav-resultados">
			...
			<div class="row">
				<div >
					<?php
					$q_buscar=mysql_query("SELECT * FROM categoria_hija WHERE nombre LIKE '%$buscar%'");
					$num_q_buscar=mysql_num_rows($q_buscar);
					$reg_q_buscar=mysql_fetch_array($q_buscar);
					$buscador=$reg_q_buscar['id_cathija'];
					$b=explode(' ', $buscar);
					$entradas=mysql_query("SELECT * FROM anuncios WHERE categoriahija_id='$buscador' or titulo LIKE '%$buscar%' or descripcion LIKE '%$buscar%' or descripcion LIKE '%$b[0]%'  ORDER BY id DESC");
			...
			<div class="row">
				...
				<div class="col-sm-9 col-xs-12">
					<div id="container-busqueda">
						...
						$publicacion=mysql_query("SELECT * FROM anuncios WHERE categoriahija_id='$buscador' or titulo LIKE '%$buscar%'  or descripcion LIKE '%$buscar%' or descripcion LIKE '%$b[0]%'ORDER BY id DESC LIMIT $inicio, $registros");
						?>
						...
						<center>
							<ul class="pagination">
								...
								if($total_registros>$registros){
									if (($pag-1)>0){
										echo "<li><a href='busqueda.php?pag=".($pag-1)."&q=$buscar'>Anterior</a></li>";
									}else{
								...
								for ($i=$pagina_desde; $i<=$pagina_hasta; $i++){
									if ($pag==$i){
										...
										echo "<li><a href='busqueda.php?pag=$i&q=$buscar'>$i</a></li>";
									}
								}

								if(($pag+1)<=$total_paginas){
									echo "<li><a href='busqueda.php?pag=".($pag+1)."&q=$buscar'>Siguiente</a></li>";
								}else{
