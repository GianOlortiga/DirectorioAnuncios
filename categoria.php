...
$q_tag=mysql_query("SELECT * FROM anuncios WHERE categoriahija_id=$tag");
$sql_cat_name=mysql_query("SELECT * FROM categoria_hija WHERE id_cathija=$tag");
	...
	<div class="container" id="nav-resultados">
			...
			<div class="row">
					<div id="categorias-busqueda" class="hidden-xs">
						<?php include("contenido/busqueda-por-categorias.php");?>
					</div>
			<hr class="hidden-xs">
			<div class="row">
				<section>
				<div class="col-sm-9">
					<div id="container-busqueda">
						...
						$publicacion=mysql_query("SELECT * FROM anuncios WHERE categoriahija_id=$tag ORDER BY id DESC LIMIT $inicio, $registros");
						?>
					...
					<div id="paginacion">
						<center>
							...
							if($total_registros>$registros){
								if (($pag-1)>0){
									echo "<li><a href='categoria.php?pag=".($pag-1)."&tag=$tag'>Anterior</a></li>";
								}else{
							...
							for ($i=$pagina_desde; $i<=$pagina_hasta; $i++){
								if ($pag==$i){
									...
									echo "<li><a href='categoria.php?pag=$i&tag=$tag'>$i</a></li>";
								}
							}

							if(($pag+1)<=$total_paginas){
								echo "<li><a href='categoria.php?pag=".($pag+1)."&tag=$tag'>Siguiente</a></li>";
							}else{
